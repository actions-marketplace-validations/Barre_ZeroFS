...
Segment reclamation observes the same checkpoints. When it first sees a dead segment, it records a deletion horizon no earlier than both:

- 60 seconds after the segment was first seen dead; and
- the longest recorded ephemeral-checkpoint span (`expire_time - create_time`) plus 30 seconds of grace, measured from the checkpoint-list observation using monotonic time.

The calculation uses the checkpoint's own timestamps, so an offset between reader and writer clocks cannot shorten this wait. Renewal updates expiry without changing creation time, so an old, repeatedly renewed checkpoint can delay segment reclamation longer than 10 minutes. Later checkpoint renewals do not extend an already recorded horizon. A running replica remains protected by advancing its view on each poll. After a replica stops, its checkpoint expires within 10 minutes; segment deletion still waits for its recorded monotonic deadline. The checkpoint lifetime and poll interval are engine defaults, not `zerofs.toml` settings.
...
- **File-data reclamation**: Tombstone cleanup, segment reclamation, and segment repacking run only on the writer. A replica never deletes or repacks segment objects. See [Space Reclamation](/space-reclamation).
