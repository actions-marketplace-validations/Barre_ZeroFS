...
<Warning>
Do not force recovery against a live or partitioned peer. It can discard an acknowledged tail held only by that process and fence its database writer. If the peer died during `Opening`, also ensure its object-store requests are quiesced. Remove `force_recovery` after the recovery startup succeeds.
</Warning>
