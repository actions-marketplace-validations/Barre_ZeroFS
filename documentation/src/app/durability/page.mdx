...
A flush uploads the open data segment before flushing metadata that refers to it. A durable manifest therefore never points to an unuploaded segment. A crash between those steps can leave an unreferenced segment object, which the orphan sweep later removes, but not a durable dangling extent pointer.
...
<Row>
  <Col>
    ...
    | tombstone cleanup | after extent delete, after tombstone update |
    | flush | after segment seal, before manifest flush |
    | segment repacking | after packed-segment seal, before repoint; between per-inode repoints |
...
The data path, namespace mutations, tombstone cleanup, segment reclamation, repacking, and crash recovery also run under deterministic simulation. Each run uses a single-threaded runtime and virtual time. A seed assigns object-store latency and therefore the order in which concurrent tasks wake; replaying that seed reproduces the schedule. Fresh seeds accumulate coverage, while a companion test verifies that replay produces the same execution trace.
...
- live reclamation cycles and concurrent repack jobs.
...
Reclamation cycles run with seeded executor tuning so the rarely taken paths execute too: checkpoint-pinned cycles, varying dead-percentage thresholds, small per-job budgets that exercise gather caps, varying job concurrency, and multi-wave drains.

A companion mode combines the simulation with failpoints for crash windows that contain no await point. It can pause between a reclamation decision and its irreversible action, allow concurrent commits to land, and then crash at that exact code location. The pointwise failpoint suite remains as a separate guard for each window.
...
| Deterministic simulation | n/a | Data and namespace models, reclamation/repack schedules, accounting invariants, and mid-flight crash recovery with exact replay |
...
When configured, ZeroFS uses Redis to coordinate conditional write operations, providing the same fencing guarantees as native conditional put support. See the [configuration reference](/configuration) for details.
