...
Set both `replication_listen` and `peers` on every node so either can lead after a role swap. ZeroFS rejects a configuration that sets only one. A standalone node may omit both; the forced-recovery procedure can temporarily remove `peers` as described below.
