...
- **`max_concurrent_compactions`** (default: 64, min: 1): Maximum number of metadata compaction operations that can run concurrently. This sets both the coordinator's scheduling limit and the embedded worker's execution limit. The scheduler starts jobs as eligible work becomes available, up to this limit. A higher limit lets independent compactions drain a backlog concurrently; lower it to reduce background CPU, memory, and object-store request use. Compaction always runs embedded in `zerofs run`; there is no separate compactor process.
...
The `[reclaim]` section tunes the segment-reclamation loop described under [Space Reclamation](/space-reclamation), not tombstone cleanup, the orphan sweep, or metadata compaction.

```toml
[reclaim]
max_concurrent_repacks = 4       # Independent segment-repack jobs (range 1-16)
repack_min_dead_percent = 10     # Repack non-small segments above this % dead (range 1-99)
```

### Parameters

- **`max_concurrent_repacks`** (default: 4, range: 1–16): Maximum number of independent source sets gathered, packed, PUT, and repointed at once. Foreground traffic does not reduce this job limit. Each job can issue bounded payload I/O, while metadata point work shares a separate global bound. Lower it to reduce object-store fan-out and memory reservations; raise it when the backend and host can sustain more parallel work.

- **`repack_min_dead_percent`** (default: 10, range: 1–99): Minimum dead percentage for repacking a segment at least 1 MiB large. Smaller segments remain candidates for consolidation. Lower values recover more space but rewrite more live data; raise it on request-billed or rewrite-sensitive backends.

Each job uses a fixed nominal selection and gather budget of 256 MiB of stored live frame data. At the default concurrency of four jobs, the aggregate budget is 1 GiB. A job may exceed its soft budget for the first source to ensure progress; the complete encoded output is limited to 1 GiB. Buffers and metadata consume additional memory. Segment uploads use one PUT rather than multipart.
